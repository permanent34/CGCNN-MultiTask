...
parser.add_argument('--num-targets', default=1, type=int, metavar='N',
                    help='number of target properties for multi-task '
    ...
    if test:
        ...
        with open('test_results.csv', 'w', newline='') as f:
            ...
            if args.task == 'classification':
                ...
                test_targets = torch.cat(test_targets, dim=0).tolist()
                test_preds = torch.cat(test_preds, dim=0).tolist()
                for cif_id, target, pred in zip(test_cif_ids, test_targets,
                                                test_preds):
                    writer.writerow((cif_id,) + tuple(target) +
                                    tuple(pred))
    else:
        star_label = '*'
    if args.task == 'regression':
...
class Normalizer(object):
    ...
    def __init__(self, tensor):
        ...
        if torch.isnan(tensor).any():
            ...
            self.mean = clean.sum(dim=0) / count
            centered = torch.where(mask, tensor - self.mean,
...
def masked_mse_loss(prediction, target):
    ...
    Used for multi-task regression where some target properties may be
    missing. Equivalent to nn.MSELoss() when target has no NaN.
    """
...
def mae(prediction, target):
    ...
    ignoring NaN entries in target.

    Parameters
    ----------

    prediction: torch.Tensor (N, K)
    target: torch.Tensor (N, K)
    """
    mask = ~torch.isnan(target)
    target_clean = torch.where(mask, target, torch.zeros_like(target))
    diff = torch.abs(target_clean - prediction) * mask
